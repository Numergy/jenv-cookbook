...
def build_script_code
  script = []
  script << %(export JENV_ROOT="#{jenv_root}")
  script << %(export PATH="${JENV_ROOT}/bin:$PATH")
  script << %(eval "$(jenv init -)")
  script << %(export JENV_VERSION="#{java_version(new_resource.jenv_version)}") if new_resource.jenv_version
